...
public:
    ...
    {
        std::filesystem::path profile_dir = sysroot / "etc/distro/profiles/default";
        ...
        // Generate /etc/profile.d/distro-channels.sh
        std::filesystem::path profile_d = sysroot / "etc/profile.d";
        std::filesystem::create_directories(profile_d);

        std::filesystem::path sh_path = profile_d / "distro-channels.sh";
        std::ofstream sh(sh_path);
        if (!sh.is_open()) {
            return std::unexpected("Cannot write /etc/profile.d/distro-channels.sh");
        }

        sh << "#!/bin/sh\n";
        sh << "# Auto-generated by Sage Package Manager Channel Runtime\n\n";
        sh << "if [ -d \"/etc/distro/profiles/default/bin\" ]; then\n";
        sh << "    case \":$PATH:\" in\n";
        sh << "        *:/etc/distro/profiles/default/bin:*);;\n";
        sh << "        *) export PATH=\"/etc/distro/profiles/default/bin:$PATH\";;\n";
        sh << "    esac\n";
        sh << "fi\n\n";

        sh << "if [ -d \"/etc/distro/profiles/default/lib\" ]; then\n";
        sh << "    case \":$LD_LIBRARY_PATH:\" in\n";
        sh << "        *:/etc/distro/profiles/default/lib:*);;\n";
        sh << "        *) export LD_LIBRARY_PATH=\"/etc/distro/profiles/default/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}\";;\n";
        sh << "    esac\n";
        sh << "fi\n";
